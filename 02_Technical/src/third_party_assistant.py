...
REPL commands:
  audit <text>          -- run the 54-pattern deception scan
  ...
  chat                  -- hand off to the agentic REPL (Ollama tool calling)
  discovery [label]     -- environment reconnaissance (uses socket)
  help                  -- show this help
  quit                  -- exit

Every command is sealed to the Merkle ledger
(vault_io.append_block -> the persistent Merkle chain) under
...
# DiscoveryAgent lives in 02_Technical/tools/ (operator CLI tool, not
# part of the runtime boundary). It is imported lazily by the REPL
# when the operator invokes the `discovery` command -- never at
# import time -- so the no-network audit on the runtime tree stays CLEAN.
# See 04_Validation/AUDIT_NO_NETWORK.md and the discovery_agent docstring.
...
REPL commands:
  audit <text>          Run the 54-pattern deception scan on <text>
  ...
  chat [prompt]         Hand off to the agentic REPL (Ollama tool calling)
  discovery [label]     Environment reconnaissance (DNS + network)
  help                  Show this help
...
================================================================================
  ...
  Audit:  Merkle truth ledger (vault_io / facts_registry)
================================================================================
...
"""
    ...
    def _dispatch(self, cmd: str, rest: List[str], raw: str) -> None:
        if cmd == "audit":
            ...
            # Lazy import -- DiscoveryAgent lives in 02_Technical/tools/
            # and uses socket for DNS probes. Importing it here, at the
            # moment the operator runs `discovery`, keeps the runtime
            # import graph free of network-touching modules. The no-
            # network audit on 02_Technical/src/ stays CLEAN.
            from tools.discovery_agent import DiscoveryAgent as _DiscoveryAgent
            ...
            # Hand off to the agentic REPL (tools/agentic_repl.py).
            # We spawn it as a subprocess so the operator gets a
            # fully interactive Ollama loop without us having to
            # import urllib.request into the runtime. Closing OPEN_ITEMS
            # D5 ("Order Get It Right as a product, not a sequence
            # of CLI invocations") is exactly this handoff.
            import subprocess
            from pathlib import Path
            repl = Path(__file__).resolve().parent.parent / "tools" / "agentic_repl.py"
            prompt_args = []
            if rest:
                # Non-interactive one-shot: agentic_repl.py --prompt "..."
                prompt_args = ["--prompt", " ".join(rest)]
            rc = subprocess.call([sys.executable, "-m", "tools.agentic_repl", *prompt_args],
                                  cwd=str(Path(__file__).resolve().parent.parent))
            if rc != 0:
                print(f"[agentic REPL exited with code {rc}]")
        else:
