...
def section_partners():
    ...
    return {
        ...
        "a": "The operator. The chain. The git log. Ollama (11 models). 5 OGIR skills. 7 agents (Hermes, Cline, opencode, codex, claude, onyx, agentic_repl). 5-allow-list (loopback only). No external network. No cloud credentials. Offline-by-policy.",
        "status": "OK",
        "evidence": "agent_stack_audit.py: 6/7 pass (codex is not on critical path). 5-allow-list enforced by test_allow_list_closed.py.",
    }
...
def section_research():
    ...
    return {
        "q": "What is known?",
        "a": f"3 sources of truth: the chain ({block_count:,} blocks, witness of every event), the git log (108 commits), the 89/100/100 calibration (2026-07-22, 118 EVAL cases). Known unknowns: industry-position claim (no market research), 5 OGIR skills adequacy (no usage metrics), agent behavior under stress (no adversarial test).",
        "status": "ATTENTION",
        "evidence": f"The 89% is from 2026-07-22, not re-run this session. Calibration report exists ({cal_exists}). Industry-position unsubstantiated. Duplicate work between audit scripts and OGIR skills.",
    }
...
def section_design():
    ...
    return {
        ...
        "a": "A legal-deception-detection system. 4 components (deception_ontology 24KB, deception_scanner 19KB, evaluation_service 3KB, legal_affidavit_generator 9KB). Plus 4 audit scripts, 31 probes, 18 questions. The product is the lie detector; the audit is the wrapper. The wrapper is overbuilt relative to the product. The 2026-07-22 directive (drop the 5-allow-list, rewrite the 5 files) is unexecuted.",
        "status": "ATTENTION",
        "evidence": f"Lie detector works (89/100/100, 118 cases). Audit infrastructure works (35/35 tests, 31/31 probes, 18/18 questions, 0 cracks). 5-allow-list contradicts operator's 2026-07-22 directive. 5 files use urllib/socket, not subprocess. WHY_THIS_FAILED.md documents the contradiction: {why_exists}.",
    }
